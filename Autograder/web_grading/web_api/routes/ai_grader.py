...
async def extract_question(session_id: int, request: ExtractQuestionRequest):
    ...
    with get_db_connection() as conn:
        ...
        if not image_data:
            raise HTTPException(status_code=500, detail="Problem image data not available")
...
@router.post("/{session_id}/generate-rubric", response_model=GenerateRubricResponse)
async def generate_rubric(session_id: int, request: GenerateRubricRequest):
    ...
    try:
        ...
        if not example_answers:
            raise HTTPException(
                status_code=400,
                detail=f"No manually graded examples found for problem {request.problem_number}. "
                       "Please manually grade at least a few submissions first."
            )

        # Generate rubric
        rubric = ai_grader.generate_rubric(
