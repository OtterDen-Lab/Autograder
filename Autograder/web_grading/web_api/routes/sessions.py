...
@router.post("/{session_id}/queue-transcriptions")
async def queue_transcriptions(session_id: int, problem_number: Optional[int] = None):
    """
    Queue all problems in a session (or specific problem number) for background transcription.

    Args:
        session_id: The grading session ID
        problem_number: Optional - if provided, only queue this problem number

    Returns:
        Status with count of problems queued
    """
    ...
    with get_db_connection() as conn:
        ...
        # Get all uncached problems
        if problem_number is not None:
            cursor.execute("""
                SELECT id FROM problems
                WHERE session_id = ? AND problem_number = ? AND transcription IS NULL
                ORDER BY submission_id
            """, (session_id, problem_number))
        else:
            cursor.execute("""
                SELECT id FROM problems
                WHERE session_id = ? AND transcription IS NULL
                ORDER BY problem_number, submission_id
            """, (session_id,))
    ...
    if not problem_ids:
        return {
            "status": "success",
            "message": "No problems need transcription (all cached)",
            "queued_count": 0
        }

    # Queue all problems with background priority
    queue = get_transcription_queue()
    for problem_id in problem_ids:
        queue.add_task(problem_id, priority=3)  # Low priority background processing

    log.info(f"Queued {len(problem_ids)} problems for transcription in session {session_id}")

    return {
        "status": "success",
        "message": f"Queued {len(problem_ids)} problems for background transcription",
        "queued_count": len(problem_ids)
    }
