    ...
    for submission in submissions:
      ...
      for problem in problems:
        ...
        if not region_coords_json:
          log.warning(
            f"Problem {problem_id} (number {problem_number}) has no region coordinates, skipping"
          )
          continue
        ...
        if qr_data:
          ...
          log.debug(
            f"Problem {problem_number} (ID {problem_id}): No QR code found")
...
async def fix_blank_counts(session_id: int):
  ...
  with get_db_connection() as conn:
    ...
    log.info(
      f"Fixed blank flags for {rows_updated} problems in session {session_id}")
...
async def rerun_blank_detection(session_id: int):
  ...
  async def event_generator():
    with get_db_connection() as conn:
      ...
      for problem_num in problem_numbers:
        try:
          ...
          blank_results = exam_processor.is_blank_heuristic_population(
            images_base64, percentile_threshold=5.0)
          ...
          for problem_id, blank_result in zip(problem_ids, blank_results):
            ...
            cursor.execute(
              ...
              (1 if blank_result["is_blank"] else 0,
               blank_result["confidence"], blank_result["method"],
               blank_result["reasoning"], problem_id))

            processed_count += 1
            # Send progress update per submission
            is_blank_val = int(
              blank_result['is_blank']) == 1  # Convert to Python bool
          ...
          log.info(
            f"Problem {problem_num}: processed {len(problem_submissions)} submissions, "
      ...
      log.info(
        f"Population-based blank detection complete: {blank_detected} blank, {not_blank} not blank, {errors} errors out of {total_problems} problems"
      )
